...
void atualizarPosicaoEscudo() {
  if (shield.ativo && escudoGirando) {
    ...
    if (shield.angle >= 360.0f)
      shield.angle -= 360.0f;  // Reseta o ângulo se ultrapassar 360 graus
  }
}
...
void inicializa() {
  ...
  carregarTextura("AngeloSprite/Atirando pra Direita - Movendo.png",
                  &playerTextures[10]);
  carregarTextura("AngeloSprite/Atirando pra Trás - Movendo.png",
                  &playerTextures[12]);
  ...
  carregarTextura("AngeloSprite/Atirando pra Direita - Movendo 2.png",
                  &playerTextures[11]);
  carregarTextura("AngeloSprite/Atirando pra Trás - Movendo 2.png",
                  &playerTextures[13]);
  ...
  carregarTextura(
      ...
      &playerTextures[17]);
  carregarTextura("AngeloSprite/Angelo com lancachamas apontando pra trás.png",
                  &playerTextures[18]);
  carregarTextura("AngeloSprite/Angelo com lancachamas apontando pra trás2.png",
                  &playerTextures[19]);
  carregarTextura(
...
void mouse(int button, int state, int x, int y) {
  ...
  if (button == GLUT_LEFT_BUTTON &&
      state == GLUT_DOWN) {  // Apenas atira se o botão esquerdo do mouse for
                             // pressionado
    ...
    if (shots[i].ativo) {
      ...
      shots[i].x += shots[i].dx *
                    shots[i].velocidade;  // Mover os tiros mais rapidamente na
                                          // direção x que ele esta olhando
      shots[i].y += shots[i].dy *
                    shots[i].velocidade;  // Mover os tiros mais rapidamente na
                                          // direção y que ele esta olhando
...
void checarColisaoEntreMosquitos() {
  for (int i = 0; i < count_mosquitoes; i++) {
    for (int j = i + 1; j < count_mosquitoes; j++) {
      ...
      if (distancia < tam) {  // Se a distância for menor que o tamanho dos
                              // mosquitos, há uma colisão
...
void timer(int value) {//  João Pedro
  if (!isPaused) {
    ...
    atualizarPosicaoJogador();  // Atualiza a posição do jogador com base nas
                                // teclas pressionadas
    ...
    if (count_time_game == 30 * aux_count_time) {
      ...
      intervalo_spawn =
          3000 /
          (level + 1);  // Exemplo: nível 0 => 3000ms, nível 1 => 1500ms, etc.
    }
    // Spawnando mosquito de acordo com o intervalo de tempo do nível de
    // infestação
